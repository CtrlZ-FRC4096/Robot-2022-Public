...
class XboxCommandController():
	...
	def __init__(self, port: int) -> None:
		...
		self.BOTH_TRIGGERS = CustomAnalog(lambda: -(self.LEFT_TRIGGER.getRaw()) if self.LEFT_TRIGGER.getRaw() > self.RIGHT_TRIGGER.getRaw() else self.RIGHT_TRIGGER.getRaw())
		self.LEFT_TRIGGER_AS_BUTTON = Button(lambda: self.LEFT_TRIGGER.getRaw() > 0.05)
		self.RIGHT_TRIGGER_AS_BUTTON = Button(lambda: self.RIGHT_TRIGGER.getRaw() > 0.05)

		self.LEFT_JOY_UP = Button(lambda: self.LEFT_JOY_Y.getRaw() < -.1)
		self.LEFT_JOY_DOWN = Button(lambda: self.LEFT_JOY_Y.getRaw() > .1)
		self.LEFT_JOY_LEFT = Button(lambda: self.LEFT_JOY_X.getRaw() < -.1)
		self.LEFT_JOY_RIGHT = Button(lambda: self.LEFT_JOY_X.getRaw() > .1)

		self.RIGHT_JOY_UP = Button(lambda: self.RIGHT_JOY_Y.getRaw() < -.1)
		self.RIGHT_JOY_DOWN = Button(lambda: self.RIGHT_JOY_Y.getRaw() > .1)
		self.RIGHT_JOY_LEFT = Button(lambda: self.RIGHT_JOY_X.getRaw() < -.1)
		self.RIGHT_JOY_RIGHT = Button(lambda: self.RIGHT_JOY_X.getRaw() > .1)
